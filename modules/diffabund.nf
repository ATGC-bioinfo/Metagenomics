process DIFFABUND {
    ...
    label  'diffabund'
    publishDir "${params.outdir}/diffabund", mode: 'copy', pattern: "*.{html,tsv,png}"
    ...
    path("deseq2_volcano.html"),      emit: volcano_plot
    path("deseq2_ma.html"),           emit: ma_plot
    path("lefse_cladogram.html"),     emit: cladogram
    path("sig_heatmap.html"),         emit: heatmap
...
# Determine group column (use 'group' or second column)
...
if len(reports) < 2:
    ...
    for f in ["deseq2_results.tsv", "ancombc_results.tsv", "lefse_results.tsv",
              "deseq2_volcano.html", "deseq2_ma.html",
              "lefse_cladogram.html", "sig_heatmap.html"]:
        with open(f, 'w') as fh:
            fh.write("")
    exit(0)

# Parse Bracken reports
samples = []
abundances = []
for r in reports:
...
abund = abund.loc[(abund.sum(axis=1) > 10)]  # Filter low-abundance taxa
...
# ── Helper: Compute log2 fold change and p-value ──────────────────────────
...
# ── 1. DESeq2-style analysis (using PyDESeq2 if available, else moderated Wald) ──
try:
    from pydeseq2.dds import DeseqDataSet
    from pydeseq2.ds import DeseqStats
    # Build count matrix
    count_df = abund.astype(int)
    count_df = count_df[count_df.sum(axis=1) > 10]  # pre-filter

    # Build metadata DataFrame for samples
...
if not has_pydeseq2 or deseq_res.empty:
    # Fallback: moderated Welch t-test
    results = []
...
fig_volcano.update_layout(
    ...
    hovermode='closest')
fig_volcano.write_html("deseq2_volcano.html")
...
fig_ma.update_layout(
    ...
    height=600, width=800, template='plotly_white',
    hovermode='closest')
fig_ma.write_html("deseq2_ma.html")

# ── 2. ANCOM-BC (Analysis of Composition of Microbiomes with Bias Correction) ──
# Simplified implementation using log-ratio approach
ancom_results = []
for taxon in abund.index:
...
# ── 3. LEfSe (Linear discriminant analysis Effect Size) ──────────────────
lefse_results = []
for taxon in abund.index:
    g1_vals = abund.loc[taxon, samples_g1].values.astype(float)
    g2_vals = abund.loc[taxon, samples_g2].values.astype(float)
    ...
    # Effect size: absolute difference in mean relative abundance
...
# Build a radial hierarchy from taxonomic names
# Parse taxon names as: k__Bacteria|p__Firmicutes|c__Bacilli|o__Lactobacillales|...
# or flat names
...
fig_clad.write_html("lefse_cladogram.html")

rel_abund = abund.div(abund.sum(axis=0), axis=1) * 100

# ── Significant Taxa Heatmap ────────────────────────────────────────────
...
if sig_taxa_list:
    ...
    if len(heat_data) > 0:
        ...
        fig_heat.update_layout(
            ...
            margin=dict(l=250))
        fig_heat.write_html("sig_heatmap.html")
    else:
        with open("sig_heatmap.html", 'w') as f:
            f.write('<html><body><h3>No significant taxa found</h3></body></html>')
else:
    with open("sig_heatmap.html", 'w') as f:
        f.write('<html><body><h3>No significant taxa found</h3></body></html>')
