...
fig_alpha.write_image("community_alpha.png", format='png', width=900, height=400)
...
fig_beta.write_image("community_beta.png", format='png', width=500, height=500)

if len(samples) >= 2:
    scaler = StandardScaler()
    X = scaler.fit_transform(rel_abund.T)
    n_pc = max(2, min(3, len(samples) - 1, X.shape[1]))
    ...
    fig_pca.update_layout(
        title=f'PCA — {var_exp[0]:.1f}% / {var_exp[1]:.1f}% variance' if len(var_exp) > 1 else f'PCA — {var_exp[0]:.1f}% variance',
        xaxis_title=f'PC1 ({var_exp[0]:.1f}%)' if len(var_exp) > 0 else 'PC1',
        yaxis_title=f'PC2 ({var_exp[1]:.1f}%)' if len(var_exp) > 1 else 'PC2',
        height=500, width=600)
    fig_pca.write_image("community_pca.png", format='png', width=600, height=500)
else:
    with open("community_pca.png", 'w') as f:
        f.write('')
...
fig_heat.update_layout(
    ...
    margin=dict(l=200))
fig_heat.write_image("community_heatmap.png", format='png', width=800, height=max(400, 20*20))
